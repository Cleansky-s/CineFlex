...
class Pelicula {
    ...
    public const EXTENSIONES_PERMITIDAS_VIDEO = array('mp4', 'webm' , 'ogg');
    ...
    public static function buscaPorTituloExacto($titulo)
    {
        $conn = Aplicacion::getInstance()->getConexionBd();
        // debido al collation que usamos no es necesario utilizar LOWER()
        $query = sprintf("SELECT * FROM peliculas WHERE titulo LIKE '%s'"
    ...
    public function setEnSuscripcion($enSuscripcion) {
        $this->enSuscripcion = $enSuscripcion;
    }
